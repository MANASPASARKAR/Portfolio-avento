...
export function TestimonialCarousel({ testimonials }: TestimonialCarouselProps) {
  ...
  return (
    <div className="relative max-w-4xl mx-auto" ref={containerRef}>
      <div 
        ref={slideRef}
        className="p-8 md:p-12 rounded-3xl border border-border bg-surface backdrop-blur-md relative shadow-sm"
        aria-live="polite"
      >
        <svg className="w-12 h-12 text-brand/20 absolute top-8 left-8" fill="currentColor" viewBox="0 0 32 32">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-brand-soft border border-brand/20 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold text-brand">
              AV
            </div>
            <div>
          ...
          {testimonials.length > 1 && (
            <div className="flex items-center gap-2">
              <button 
                onClick={prev}
                className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-surface-muted transition-colors text-foreground"
